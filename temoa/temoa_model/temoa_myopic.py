...
def myopic_db_generator_solver(self):
    ...
    time_periods = cur_org.execute("SELECT period FROM TimePeriod WHERE flag='f'").fetchall()
    ...
    for i in range(N - 1, len(time_periods) - 1):
        ...
        cur.execute("DELETE FROM Efficiency WHERE tech IN (SELECT tech FROM LifetimeProcess WHERE \
                     LifetimeProcess.lifetime+LifetimeProcess.vintage<=" + str(time_periods[i - (N - 1)][0]) + ") \
                     AND vintage IN (SELECT vintage FROM LifetimeProcess WHERE LifetimeProcess.lifetime+\
        ...
        FROM Efficiency INNER JOIN LifetimeTech ON (LifetimeTech.region=Efficiency.regions AND LifetimeTech.tech=Efficiency.tech) \
        WHERE Efficiency.vintage + LifetimeTech.lifetime <= " + str(time_periods[i - (N - 1)][0]) + ")\
        ...
        while len(cur.execute("SELECT * FROM Efficiency WHERE output_comm NOT IN (SELECT input_comm FROM Efficiency)\
                               AND output_comm NOT IN (SELECT name FROM Commodity WHERE flag='d');").fetchall()) > 0:

            cur.execute("DELETE FROM Efficiency WHERE output_comm NOT IN (SELECT input_comm FROM Efficiency) \
                     AND output_comm NOT IN (SELECT name FROM Commodity WHERE flag='d');")
            iterval += 1
            if iterval > 10:
                break
        ...
        cur.execute("DELETE FROM TimePeriod WHERE period > " + str(time_periods[i][0]) + ";")
            ...
            if i != (N - 1):
                ...
                for ind, row in df_linkedtechs.iterrows():
                    primary_tech = pd.read_sql_query(
                        "SELECT primary_tech FROM LinkedTech WHERE primary_region = '" + row['regions'] + \
                ...
                for ind, row in df_primarytechs.iterrows():
                    linked_tech = pd.read_sql_query(
                        "SELECT driven_tech FROM LinkedTech WHERE primary_region = '" + row['regions'] + \
        ...
        if i != (N - 1):
            df_new_ExistingCapacity = pd.read_sql_query("SELECT region, tech, vintage, capacity FROM OutputNetCapacity \
                                                         WHERE scenario=" + "'" + str(self.options.scenario) + "' AND \
            ...
            cur_org.execute("DELETE FROM OutputEmission WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM OutputFlowIn WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM OutputFlowOut WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM OutputNetCapacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM OutputBuiltCapacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND vintage>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM OutputRetiredCapacity WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            cur_org.execute("DELETE FROM OutputCurtailment WHERE scenario=" + "'" + str(
                self.options.scenario) + "' AND t_periods>" + str(time_periods[i - N][0]))
            con_org.commit()
        ...
        for table in table_list:
            ...
            try:
                if table[0] == 'LinkedTechs':
                    cur.execute(
                        "DELETE FROM LinkedTech WHERE primary_tech NOT IN (SELECT DISTINCT(tech) FROM Efficiency)")
                    cur.execute(
                        "DELETE FROM LinkedTech WHERE driven_tech NOT IN (SELECT DISTINCT(tech) FROM Efficiency)")
                ...
                if 'vintage' in names:
                    ...
                    if table[0] == 'EmissionActivity':
                        cur.execute("DELETE FROM EmissionActivity WHERE region || input_comm || tech || vintage || output_comm \
                                    NOT IN (SELECT region || input_comm || tech || vintage || output_comm FROM Efficiency)")
                    else:
        ...
        cur.execute("UPDATE Commodity SET name = TRIM(name, CHAR(10,13,37))")
        # delete unused commodities otherwise the model throws an error

        cur.execute(
            "DELETE FROM Commodity WHERE flag!='e' AND name NOT IN (SELECT input_comm from Efficiency UNION SELECT output_comm from Efficiency);")
        cur.execute("INSERT INTO TimePeriod (period,flag) VALUES (" + str(time_periods[i + 1][0]) + ",'f');")
        cur.execute("UPDATE TimePeriod SET flag='e' WHERE period < " + str(time_periods[i - (N - 1)][0]))
        ...
        if (i != (N - 1)) & ('MaxResource' in [x[0] for x in table_list]):
            resource_constraints_org = pd.read_sql_query("SELECT region, tech, max_res FROM MaxResource", con_org)
            for ind, row in resource_constraints_org.iterrows():
                df_existing_resources = pd.read_sql_query("SELECT sum(flow) FROM OutputFlowOut \
                                                        WHERE region='" + row['regions'] + "' AND \
                                                        tech='" + row['tech'] + "' AND \
                ...
                try:
                    ...
                    query = "UPDATE MaxResource SET max_res=" + str(updated_resource) + " WHERE regions='" \
